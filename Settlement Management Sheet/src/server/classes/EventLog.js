export default class EventLog {
  ...
  calculateEventImpact(event, settlement) {
    return event.impacts.map(({ category, attribute, quantity, scaling }) => {
      // Severity multiplier
      ...
      // Clamp dependency modifier to prevent extremes
      dependencyModifier = Math.min(Math.max(dependencyModifier, 0.5), 3);

      // Final impact calculation
      const finalImpact = quantity * severityMultiplier * dependencyModifier;

      return {
        category,
        attribute,
        finalImpact: Math.round(this.clampImpact(finalImpact)),
      };
    });
  }
