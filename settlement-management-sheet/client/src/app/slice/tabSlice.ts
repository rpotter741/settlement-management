...
const tabSlice = createSlice({
  ...
  reducers: {
    ...
    removeTab: (state, action: PayloadAction<RemoveTabPayload>) => {
      ...
      removeTabFn({ state, tabId, side, preventSplit });
    },
    ...
    bulkRemoveTabs: (state, action: PayloadAction<{ objectIds: string[] }>) => {
      const { objectIds } = action.payload;
      const leftTabs = Object.values(state.left.data);
      const rightTabs = Object.values(state.right.data);
      objectIds.forEach((tabId) => {
        const leftTab = leftTabs.find((tab) => tab.id === tabId);
        if (leftTab) {
          removeTabFn({ state, tabId: leftTab.tabId, side: 'left' });
        } else {
          const rightTab = rightTabs.find((tab) => tab.id === tabId);
          if (rightTab) {
            removeTabFn({ state, tabId: rightTab.tabId, side: 'right' });
          }
        }
      });
    },
...
function removeTabFn({
  ...
  if (side === 'left' || state.preventSplit) {
    state.left.order = state.left.order.filter((id: string) => id !== tabId);
    if (state.left.currentId === tabId) {
    ...
    if (state.left.order.length === 0 && state.right.order.length !== 0) {
      state.splitTabs = false;
      state.preventSplit = false;
      state.right.order.forEach((id: string) => {
    ...
    state.right.order = state.right.order.filter((id: string) => id !== tabId);
