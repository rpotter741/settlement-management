...
import useClipboard from '@/hooks/global/useClipboard.js';
import changeSubTypePropertyThunk from '@/app/thunks/glossary/subtypes/changeSubTypePropertyThunk.js';
import { v4 as newId } from 'uuid';
import changeSubTypeGroupThunk from '@/app/thunks/glossary/subtypes/changeSubTypeGroupThunk.js';
...
const SubTypeSidebarEditor = ({
  ...
  const { data, isCompatible, setClipboard } = useClipboard([
    'subType-property',
    'subType-group',
  ]);
  ...
  const handleContextMenu = (
    ...
    setContextMenu({
      ...
      propertyType: propertyId
        ...
        : null,
      inputType: propertyId
        ? subType.groupData[groupId].propertyData[propertyId].inputType
        : null,
    });
  };
  ...
  return (
    <Box>
      ...
      <Box sx={{ display: 'flex', gap: 0, justifyContent: 'center', mt: 2 }}>
        ...
        <IconButton
          sx={{
            color: showSettings ? 'secondary.light' : 'text.primary',
          }}
      ...
      {contextMenu && (
        ...
        >
            ...
            onClick={() => {
              contextMenu.propertyId
                ? setClipboard({
                    ...
                    data: {
                      ...cloneDeep(
                        subType?.groupData[contextMenu.groupId || '']
                          .propertyData[contextMenu.propertyId || '']
                      ),
                    },
                  })
                : setClipboard({
          ...
          <MenuItem
            disabled={
              contextMenu.propertyId
                ? !isCompatible['subType-property']
                : !isCompatible['subType-group']
            }
            onClick={() => {
              if (!data || !contextMenu) return;
              const newGroupOrProperty = cloneDeep(data);
              newGroupOrProperty.id = newId();
              contextMenu.propertyId
                ? changeSubTypePropertyThunk({
                    subTypeId: editId,
                    groupId: contextMenu.groupId || '',
                    propertyId: contextMenu.propertyId || '',
                    property: newGroupOrProperty,
                  })
                : changeSubTypeGroupThunk({
                    subTypeId: editId,
                    groupId: contextMenu.groupId || '',
                    group: newGroupOrProperty,
                  });
              closeContextMenu();
            }}
          >
