...
use crate::types::*;
use crate::{
    entities::{backlink_index, entry_sub_type, glossary_entry, sub_type_group, sub_type_property},
...
pub async fn update_entry_groups(
    ...
    entry: glossary_entry::Model,
) -> Result<GlossaryEntry, String> {
    /*---------------------------------------------------------------------- */
    /*-----------------Getting sub type, groups, and properties ------------ */
    /*---------------------------------------------------------------------- */
    let _existing_subtype = entry_sub_type::Entity::find()
        .filter(entry_sub_type::Column::Id.eq(entry.sub_type_id.clone()))
        .one(db.inner())
        .await
    ...
    let _existing_entry = glossary_entry::Entity::find()
        .filter(glossary_entry::Column::Id.eq(entry.id.clone()))
        .one(db.inner())
        .await
        .map_err(|e| e.to_string())?
        .ok_or_else(|| "Glossary entry not found".to_string())?;

    let _entry_groups: HashMap<String, BasicGroup> =
        serde_json::from_value(serde_json::Value::from(entry.groups.clone()))
            .map_err(|e| e.to_string())?;
    ...
    for added in added_ids {
        let active_model = backlink_index::ActiveModel {
            id: Set(added.id),
        ...
        backlink_index::Entity::insert(active_model)
            .exec(&txn)
            .await
            .map_err(|e| e.to_string())?;
    }
    ...
    for removed in removed_ids {
        backlink_index::Entity::delete_many()
            .filter(backlink_index::Column::Id.eq(removed.id))
            .exec(&txn)
            .await
            .map_err(|e| e.to_string())?;
    }

    let mut active_entry: glossary_entry::ActiveModel = _existing_entry.into();
    active_entry.groups = Set(entry.groups);

    let updated_entry = glossary_entry::Entity::update(active_entry)
        .exec(&txn)
        .await
        .map_err(|e| e.to_string())?;
    ...
    Ok(GlossaryEntry::try_from(updated_entry)?)
}
...
pub fn extract_relationship_ids(
    ...
    groups_and_properties: &(Vec<sub_type_group::Model>, Vec<sub_type_property::Model>),
