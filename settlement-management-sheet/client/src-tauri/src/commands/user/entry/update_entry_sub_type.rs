use crate::entities::{user_glossary_entry, user_glossary_node, user_sub_type};
...
use crate::types::UserGlossaryEntry;
use sea_orm::{
...
pub async fn update_entry_sub_type(
    ...
    let _new_sub_type = user_sub_type::Entity::find()
        .filter(user_sub_type::Column::Id.eq(sub_type_id.0.clone()))
        .one(db.inner())
        .await
        .map_err(|e| e.to_string())?
        .ok_or_else(|| "Entry sub type not found".to_string())?;

    let existing_model = user_glossary_entry::Entity::find()
        .filter(user_glossary_entry::Column::Id.eq(entry_id.0.clone()))
        .one(db.inner())
        .await
        .map_err(|e| e.to_string())?
        .ok_or_else(|| "Glossary entry not found".to_string())?;

    let _existing_entry: UserGlossaryEntry = UserGlossaryEntry::try_from(existing_model)?;
    ...
    let updated_entry = user_glossary_entry::ActiveModel {
    ...
    let update_node = user_glossary_node::ActiveModel {
        id: Set(entry_id.0.clone()),
        sub_type_id: Set(sub_type_id.0.clone()),
        ..Default::default()
    };
